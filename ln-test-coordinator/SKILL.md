---
name: ln-test-coordinator
description: Plans Story test task by Risk-Based Testing after manual testing. Calculates priorities, selects E2E/Integration/Unit, delegates to ln-task-creator. Invoked by ln-story-quality-coordinator.
---
...
- **Invoked by ln-story-quality-coordinator Pass 1** after manual functional testing PASSED
- **Invocation method:** Use Skill tool with command: `Skill(command: "ln-test-coordinator")`
...
- ln-story-quality-coordinator Pass 1 completed manual testing
- Manual test results in Linear comment (created by ln-story-quality-coordinator Phase 3 step 4)

**Automation:** Supports `autoApprove: true` (default when invoked by ln-story-quality-coordinator) to skip manual confirmation and run unattended.

## When NOT to Use

Do NOT use if:
- Manual testing NOT completed → Wait for ln-story-quality-coordinator Pass 1
- Manual test results NOT in Linear comment → ln-story-quality-coordinator must document first
...
3. Load manual test results comment (format: ln-manual-tester Format v1.0)
   - Search for the header containing "Manual Testing Results" (see `ln-manual-tester/references/test_result_format_v1.md`)
   - If not found → ERROR: Run ln-story-quality-coordinator Pass 1 first
...
- **autoApprove: true** (default for ln-story-quality-coordinator) → proceed automatically with no user input
...
Invoke ln-task-creator worker with taskType: "test"
...
Invoke ln-task-replanner worker with taskType: "test"
...
- [ ] CREATE MODE (if count = 0): Delegated to ln-task-creator with taskType: "test"
- [ ] REPLAN MODE (if count ≥ 1): Delegated to ln-task-replanner with taskType: "test"
...
- [ ] ln-task-creator: Test task created in Linear + kanban_board.md updated
...
**Invocation (by ln-story-quality-coordinator via Skill tool):**
```
Skill(skill: "ln-test-coordinator", storyId: "US001")
```
...
**Location**: [ln-test-coordinator/references/risk_based_testing_guide.md](references/risk_based_testing_guide.md)

**Ownership**: ln-test-coordinator (orchestrator-specific logic)

**Usage**: ln-test-coordinator uses this guide in Phase 4 (Risk-Based Test Planning)

### test_task_template.md (MOVED)

**Purpose**: Story finalizer task structure (11 sections: tests + fixes + infrastructure + docs + cleanup)

**Location**: Moved to [ln-task-creator/references/test_task_template.md](../ln-task-creator/references/test_task_template.md)
...
**Usage**: Workers (ln-task-creator, ln-task-replanner) read this template when generating test task documents (via `taskType: "test"`)

### linear_integration.md (Shared Reference)

**Location**: [ln-epic-creator/references/linear_integration.md](../ln-epic-creator/references/linear_integration.md)
