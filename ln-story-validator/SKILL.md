...
- Does NOT execute implementation (delegates to ln-story-coordinator)
- Does NOT create/update ADRs (delegates to ln-adr-creator)
...
3. **Check documentation gaps:**
   ...
   c) **Auto-create missing documentation:**
      - **Guides:** Invoke ln-guide-creator (HOW to implement patterns)
      - **Manuals:** Invoke ln-manual-creator (HOW to use libraries/APIs)
      - **ADRs:** Invoke ln-adr-creator (WHY decisions - AUTO-RESEARCH from Story)
...
10. **Test Task Cleanup (#10)** - Remove premature test tasks (created later by ln-test-coordinator)
...
13. **Story Size & Task Granularity (#9)** - Check 3-8 tasks, 3-5h each; invoke ln-task-coordinator if < 3 tasks
...
**Reference:** See `ln-test-coordinator/references/risk_based_testing_guide.md` for complete methodology.
...
- No test task exists (created later by ln-test-coordinator)

**Auto-fix actions:**
- **If < 3 tasks:**
  - Analyze Story Context and Technical Notes
  - Check Consumer-First gaps (Consumer → Service → Repository)
  - Invoke ln-task-coordinator via Skill tool: `Skill(skill: "ln-task-coordinator")`
  ...
  - Add comment: "Missing implementation tasks created via ln-task-coordinator"
...
- For EACH test task found:
  ...
  - Add comment: "Test task removed - will be created by ln-test-coordinator after manual testing (Pass 1)"
...
**Rationale:** Test task created AFTER manual testing (by ln-test-coordinator), not before. Manual testing reveals real scenarios for comprehensive test coverage.
...
**Purpose:** ln-task-executor reads these links when implementing.
...
- Phase 3 (#9): Detect < 3 tasks violation
  ...
  - Invoke ln-task-coordinator: `Skill(skill: "ln-task-coordinator")`
  - ln-task-coordinator creates 2 missing tasks (Service, Repository)
...
- `../ln-task-coordinator/references/task_template_universal.md` - Task structure (7 sections)

**Checklists (Verification):**
- `verification_checklist.md` - Complete 16-criteria checklist with detailed checks

**Methodology (Testing):**
- `../ln-test-coordinator/references/risk_based_testing_guide.md` - Risk-Based Testing approach (Priority ≥15 scenarios)

**Integration (Linear API):**
- `../ln-epic-creator/references/linear_integration.md` - Linear MCP usage patterns
...
4. **Delegate task creation** - Use ln-task-coordinator Skill tool (Phase 3 #9), never create tasks directly
...
- [ ] Missing guides auto-created via ln-guide-creator
...
- [ ] **Workflow Optimization (9-12):**
  ...
  - Test tasks removed (created later by ln-test-coordinator) (#10)
  - Documentation integrated into implementation tasks (#8)
  - Tasks reordered per Consumer-First principle (#14)
- [ ] **Scope & Quality (13-16):**
  - Story size validated (3-8 tasks, ln-task-coordinator invoked if < 3) (#9)
