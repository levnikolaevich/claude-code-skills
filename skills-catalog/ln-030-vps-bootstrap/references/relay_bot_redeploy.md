...
Restart `${SERVICE_PREFIX}-god.service` only when hooks, project-scope `.claude/settings.json`, or god-session instructions changed. Relay-bot source-only changes require only `${SERVICE_PREFIX}-relay-bot.service` restart.
