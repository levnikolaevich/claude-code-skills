---
name: ln-60-docs-system
description: Creates complete doc system via ln-61-docs-creator + ln-62-html-builder + ln-22-adr-creator. Used when starting new project and need full documentation suite at once.
---
...
1. [ln-61-docs-creator](../ln-61-docs-creator/SKILL.md) - Create MD docs
2. [ln-62-html-builder](../ln-62-html-builder/SKILL.md) - Create HTML presentation (optional)
3. [ln-22-adr-creator](../ln-22-adr-creator/SKILL.md) - Add individual ADRs later (optional)
...
1. Show user what will be created:
   - Project documentation (MD files via ln-61-docs-creator)
   - HTML presentation (via ln-62-html-builder)
...
**Invocation:** `Skill(skill: "ln-61-docs-creator")` → AUTOMATIC
...
**Invocation:** `Skill(skill: "ln-62-html-builder")` → AUTOMATIC (no user input)

**Expected output:** `presentation_final.html` + `assets/` directory

**Skip:** If "no" → run ln-62-html-builder later manually
...
3. Recommend next steps:
   ...
   - "Run ln-70-epic-creator to decompose scope into Epics"
   - "Share documentation with technical stakeholders"
   - "Run ln-22-adr-creator skill to add more ADRs later (if needed)"
...
│   └── ...
├── presentation_final.html           # ← Interactive HTML (optional, generated by ln-62-html-builder)
└── assets/                           # ← HTML modules (optional, generated by ln-62-html-builder)
```
...
1. **ln-60-docs-system** (this skill) - Create complete documentation system
2. **ln-70-epic-creator** - Decompose scope into Epics (Linear Projects)
3. **ln-71-story-manager** - Create User Stories for each Epic (automatic decomposition + replan)
4. **ln-10-story-decomposer** - Break down Stories into implementation tasks (automatic decomposition + replan)
5. **ln-20-story-validator** - Verify Stories before development
6. **ln-30-story-executor** - Orchestrate Story implementation
7. **ln-40-story-quality-gate** - Review completed Stories
...
**Trade-offs:**
- ⚠️ Less granular control (can't skip ln-61-docs-creator phases)
- ⚠️ Longer execution time (15-20 minutes)
- ⚠️ Can't reuse existing docs (starts fresh)

**When to use manual approach instead:**
- Need to update existing documentation → use [ln-63-docs-updater](../ln-63-docs-updater/SKILL.md)
- Need only HTML rebuild → use [ln-62-html-builder](../ln-62-html-builder/SKILL.md)
- Need one specific ADR → use [ln-22-adr-creator](../ln-22-adr-creator/SKILL.md)
...
- [ ] Workflow explained to user:
  - Project documentation (MD files via ln-61-docs-creator)
  - HTML presentation (via ln-62-html-builder, optional)
  - Estimated time: 15-20 minutes with interactive dialog
...
- [ ] User notified: "Invoking ln-61-docs-creator skill..."
- [ ] ln-61-docs-creator invoked via Skill tool (command: "ln-61-docs-creator")
- [ ] ln-61-docs-creator completed all 5 phases:
...
- [ ] **If user chose "yes" for HTML:**
  - User notified: "Invoking ln-62-html-builder skill..."
  - ln-62-html-builder invoked via Skill tool (command: "ln-62-html-builder")
  - ln-62-html-builder completed all 3 phases:
...
- [ ] **If user chose "no":**
  ...
  - User notified: "Skipped HTML presentation (can run ln-62-html-builder later)"
...
- [ ] Next steps recommended:
  ...
  - "Share documentation with technical stakeholders"
  - "Run ln-22-adr-creator skill to add more ADRs later (if needed)"
- [ ] Integration workflow mentioned (ln-60-docs-system → ln-70-epic-creator → ln-71-story-manager → ...)
...
- Next steps recommendation (ln-70-epic-creator to decompose scope)
