...
**Note**: This skill generates MD documentation only. For interactive HTML presentation, use [ln-62-html-builder](../ln-62-html-builder/SKILL.md) skill after running this skill.
...
- **Note:** ADR template NOT copied (ln-22-adr-creator uses its own template)
...
- If yes:
  ...
  - Read template: `ln-61-docs-creator/references/kanban_board_template.md`
...
- If yes: Invoke ln-62-html-builder → WAIT completion → verify output (`presentation_final.html` + `assets/`)
- If no: Skip (can run ln-62-html-builder later)
...
ln-61-docs-creator → [optional: ln-62-html-builder] → ln-70-epic-creator → ln-71-story-manager → ln-10-story-decomposer → ln-31-task-executor → ln-32-task-reviewer → test planning → ln-34-test-executor → ln-32-task-reviewer → Story Done

**Dependencies**: ln-62-html-builder reads MD docs from docs/project/, epic-creator uses requirements.md, task-creator uses architecture.md + technical_specification.md
...
- [ ] Next step recommended: "Run ln-70-epic-creator skill to decompose scope into Epics"
