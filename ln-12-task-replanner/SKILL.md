...
- **Implementation tasks** (from ln-10-story-decomposer): Feature development tasks when Story AC/Technical Approach changed
- **Refactoring tasks** (from ln-40-story-quality-gate): Code quality improvement tasks when new issues found
- **Test tasks** (from ln-50-story-test-planner): E2E/Integration/Unit test tasks when manual test results changed
...
**1. ln-10-story-decomposer (REPLAN MODE):**
...
**3. ln-50-story-test-planner (Phase 6 - Manual Testing Changed):**
...
Read template from ln-11-task-creator/references/ based on `taskType` parameter:
...
Generate new doc (taskType determines template) → Validate (same rules as ln-11-task-creator Phase 2) → `update_issue(description)` → Add comment
...
**Validation rules (type-specific, same as ln-11-task-creator Phase 2):**
...
**Same validation logic as ln-11-task-creator Phase 2.**
...
**Location**: [ln-12-task-replanner/references/replan_algorithm.md](references/replan_algorithm.md)

**Ownership**: ln-12-task-replanner (worker-specific logic)

**Usage**: ln-12-task-replanner applies this algorithm in Phase 2 (Compare IDEAL vs Existing)

### Task Templates (ln-11-task-creator Ownership)

**This skill reads ALL 3 templates** from ln-11-task-creator/references/ directory:

**task_template_implementation.md:**
- **Purpose**: Implementation task template (7 sections, 146 lines)
- **Location**: [ln-11-task-creator/references/task_template_implementation.md](../ln-11-task-creator/references/task_template_implementation.md)
...
- **Location**: [ln-11-task-creator/references/refactoring_task_template.md](../ln-11-task-creator/references/refactoring_task_template.md)
...
- **Location**: [ln-11-task-creator/references/test_task_template.md](../ln-11-task-creator/references/test_task_template.md)
...
|--------------|---------|----------|------------------|
| ln-10-story-decomposer | Story AC/approach changed | implementation | idealPlan, guideLinks |
| ln-40-story-quality-gate | New code quality issues | refactoring | codeQualityIssues, refactoringPlan, affectedComponents |
| ln-50-story-test-planner | Manual test results changed | test | manualTestResults, testPlan, infra, docs, cleanup |
