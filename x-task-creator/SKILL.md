...
**Universal factory** worker skill for creating ALL 3 task types in Linear. Invoked by orchestrators (x-task-coordinator, x-story-quality-coordinator, x-test-coordinator) when new tasks needed.
...
- **Implementation tasks** (from x-task-coordinator): Feature development with Technical Approach
- **Refactoring tasks** (from x-story-quality-coordinator): Code quality improvements with Regression Testing
- **Test tasks** (from x-test-coordinator): E2E/Integration/Unit tests with Risk Priority Matrix
...
1. **x-task-coordinator** (CREATE MODE):
...
2. **x-story-quality-coordinator** (Path B - Issues Found):
...
3. **x-test-coordinator** (Pass 1 - Manual Testing Passed):
...
- **Manual Test Results**: Structured comment (Format v1.0) from x-story-quality-coordinator Pass 1 with AC, Test Results, Edge Cases, Error Handling, Integration
...
**Details:** Each template in `references/` directory (owned by x-task-creator). See template files for complete section descriptions.
...
**Same validation logic as x-task-replanner Phase 5.**
...
- Rationale: All new tests created by x-test-coordinator after manual testing
...
**Rationale**: Code is written during execution (x-task-executor, x-task-rework, x-test-executor), not during planning.
...
- [ ] Next steps provided (x-story-validator, x-story-coordinator)
...
- **Used for**: Feature development tasks (from x-task-coordinator)
...
- **Used for**: Code quality improvement tasks (from x-story-quality-coordinator)
...
- **Used for**: Story's final test task (from x-test-coordinator)
...
Phase 4a: Delegate to x-task-creator
...
**3. x-test-coordinator (Phase 6 - After Manual Testing):**
```javascript
Phase 5: Test Plan Analysis → Priority ≥15 scenarios selected
Phase 6: Invoke x-task-creator
  Skill(skill: "x-task-creator", {
    taskType: "test",
    teamId: teamId,
    storyData: {...},
...
- Analyze Story complexity (x-task-coordinator does this)
- Analyze code quality (x-story-quality-coordinator does this)
- Perform manual testing (x-story-quality-coordinator does this)
...
**State**: Always "Backlog" (tasks move to Todo after x-story-validator approval)
