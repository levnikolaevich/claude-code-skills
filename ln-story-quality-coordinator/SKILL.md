---
name: ln-story-quality-coordinator
...
- **Step 3:** Manual functional testing against Story AC (delegated to `ln-manual-tester`) - FAIL FAST
- **Verdict:** Create test task (via ln-test-coordinator) OR refactoring/fix task
...
- Invoked by ln-story-coordinator automatically via Skill tool when test task Done
...
Implementation Tasks (Done)
         ↓
ln-story-quality-coordinator Pass 1 (manual functional testing + code analysis)
         ↓
    [Path A: No issues]
         ↓
ln-test-coordinator (creates test task with 11 sections)
         ↓
ln-test-executor (Fix Tests → New Tests → Infrastructure → Docs → Cleanup)
         ↓
ln-task-reviewer (reviews test task)
         ↓
Test task Done
         ↓
ln-story-quality-coordinator Pass 2 (verify tests → Story Done)

    [Path B: Issues found]
         ↓
Create refactoring task → Execute → ln-story-quality-coordinator Pass 1 again
```

**Critical:** Pass 2 is automatically invoked by ln-story-coordinator when test task status = Done. Can also be invoked manually by user for re-verification.

### Usage Examples

**Automatic invocation (recommended):**
```
# Pass 1 - invoked by ln-story-coordinator when all impl tasks Done
Skill(skill: "ln-story-quality-coordinator", storyId: "US004")

# Pass 2 - invoked by ln-story-coordinator when test task Done
Skill(skill: "ln-story-quality-coordinator", storyId: "US004")
```
...
Skill(skill: "ln-story-quality-coordinator", storyId: "US004", pass: 1)

# Force Pass 2 (verify tests + mark Story Done)
Skill(skill: "ln-story-quality-coordinator", storyId: "US004", pass: 2)
```
...
- Re-run Pass 1 after fixing issues (without waiting for ln-story-coordinator)
...
**Delegate to ln-code-quality-checker (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-code-quality-checker", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "ISSUES_FOUND") + Linear comment ID
- Details: See [ln-code-quality-checker/SKILL.md](../ln-code-quality-checker/SKILL.md)

### Phase 3 Verdict: Early Exit Decision

**Parse ln-code-quality-checker result:**
...
1. **Create refactoring task:**
   ...
   - Description: Copy issues from ln-code-quality-checker result
   - Label: "refactoring"
   - Parent: Story.id
   - Status: Backlog
...
**Delegate to ln-regression-checker (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-regression-checker", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "FAIL") + Linear comment ID
- Details: See [ln-regression-checker/SKILL.md](../ln-regression-checker/SKILL.md)

### Phase 4 Verdict: Early Exit Decision

**Parse ln-regression-checker result:**
...
1. **Create regression fix task:**
   ...
   - Description: Copy failed test names from ln-regression-checker result
   - Label: "bug"
   - Parent: Story.id
   - Status: Backlog
...
**Delegate to ln-manual-tester (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-manual-tester", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "FAIL") + Linear comment ID (Format v1.0) + temporary script path
- Details: See [ln-manual-tester/SKILL.md](../ln-manual-tester/SKILL.md)

### Phase 5 Verdict: Early Exit Decision

**Parse ln-manual-tester result:**
...
1. **Create bug fix task:**
   ...
   - Description: Copy failed AC and bug details from ln-manual-tester result
   - Label: "bug"
   - Parent: Story.id
   - Status: Backlog
...
1. **Check for existing test task:**
   ...
   - **If test task exists:**
     * Check test task status:
       - **Done** → Report: "Test task [TASK-ID] Done. Pass 1 complete. ln-story-coordinator will automatically invoke Pass 2."
       - **In Progress / To Review / Todo** → Report to user: "Test task [TASK-ID] already exists (status: [STATUS]). Wait for completion before running ln-story-quality-coordinator Pass 1 again."
...
2. **Create test task (ONLY if no test task exists):**
   ...
   - **AUTOMATIC invocation:** Invoke ln-test-coordinator with `autoApprove: true` (Skill tool) → WAIT completion → test task created (no prompts)
   - Story remains current state until test task Done

3. **Next steps after ln-test-coordinator completes:**
   - Test task created in Linear and kanban_board.md
   - Execute test task via ln-test-executor (6 steps):
   ...
   - Review test task via ln-task-reviewer
   - When test task Done → ln-story-coordinator automatically invokes ln-story-quality-coordinator Pass 2
...
1. **All tests pass:**
   ...
   - Verify 0 failures
   - Verify total tests 10-28 (within enforced limit per `ln-test-coordinator/references/risk_based_testing_guide.md`)
...
2. **Update kanban_board.md (minimal cleanup):**
   - Story header already in "### Done (Last 5 tasks)" section (tasks were moved there by ln-task-reviewer)
   ...
   - No additional movement needed (tasks already moved by ln-task-reviewer)
...
4. **After fix tasks Done:** ln-story-coordinator automatically re-invokes ln-story-quality-coordinator Pass 2
...
**ln-story-quality-coordinator responsibility**: Manages **1 of 4** Story status transitions (Final approval phase).

| Story Status Transition | Responsible Skill | When |
|-------------------------|-------------------|------|
| Backlog → Todo | ln-story-validator | After auto-fix and approval |
| Todo → In Progress | ln-story-coordinator | First task execution starts |
| In Progress → To Review | ln-story-coordinator | All tasks Done |
| **To Review → Done** | **ln-story-quality-coordinator Pass 2** | **All tests verified, Priority ≥15 covered** |

**Why this matters**:
- ln-story-quality-coordinator ONLY updates status in Pass 2 (Phase 3: Mark Story Done)
- ln-story-quality-coordinator does NOT handle other transitions (Backlog → Todo, Todo → In Progress, In Progress → To Review)
- Clear ownership prevents duplicate updates from multiple skills
- ln-story-quality-coordinator Pass 1 does NOT update Story status - it creates test/refactor tasks only

### L2→L2 Delegation: Invoked by ln-story-coordinator
...
|------------|------|-------------------|-----------|
| **ln-story-coordinator → ln-story-quality-coordinator Pass 1** | After all implementation tasks Done | Task execution → Story quality validation | ln-story-coordinator delegates quality verification (Code Quality, Regression, Manual Testing, Early Exit Pattern) |
| **ln-story-coordinator → ln-story-quality-coordinator Pass 2** | After test task Done | Task execution → Final approval | ln-story-coordinator delegates final Story approval (test verification, Priority ≥15 coverage, Story → Done) |
...
- Pass 1 Phase 5: Delegates manual testing to ln-manual-tester (curl/puppeteer, Linear comments captured there)
- Pass 1 Phase 6: **Delegates to ln-test-coordinator** via Skill tool (test task creation)
- Orchestrates quality validation flow with Early Exit Pattern (fail fast at each phase)

**Compliance with L2→L2 Rules:**
- ✅ Rule 1: Explicit Skill tool invocation (invoked by ln-story-coordinator via `Skill(skill: "ln-story-quality-coordinator")`)
...
- ✅ Rule 4: DAG only (never calls back to ln-story-coordinator, only creates tasks for it to execute)
- ✅ Rule 5: Documented in both SKILL.md files

**Self-Healing Pipeline Integration:**
- Pass 1 creates fix/refactor/test tasks in Backlog → ln-story-coordinator loops back to Phase 3
- Pass 2 creates fix tasks if tests fail → ln-story-coordinator loops back to Phase 3
...
- [ ] ln-code-quality-checker invoked via Skill tool
...
- [ ] ln-regression-checker invoked via Skill tool
...
- [ ] ln-manual-tester invoked via Skill tool
- [ ] Worker result received: JSON verdict ("PASS" | "FAIL")
...
- [ ] **If test task does NOT exist:** Proceeded to ln-test-coordinator invocation

**✅ Verdict Determined (Phase 6):**
- [ ] **All Phase 3-4-5 passed** (Code Quality → Regression → Manual Testing)
- [ ] ln-test-coordinator invoked AUTOMATICALLY with autoApprove: true (ONLY if no test task exists)
...
- [ ] **Pass:**
  ...
  - kanban_board.md minimal cleanup:
    * Story header already in "### Done (Last 5 tasks)" section (tasks moved there by ln-task-reviewer)
...
5. **Phase 6: E2E first** - Use Skill tool to invoke ln-test-coordinator (Risk-Based Testing)
...
| Aspect | ln-story-validator | ln-task-reviewer | ln-story-quality-coordinator Pass 1 | ln-story-quality-coordinator Pass 2 |
|--------|-------------------|-----------------|-------------------------|-------------------------|
