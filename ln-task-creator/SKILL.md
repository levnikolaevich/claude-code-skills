...
**Universal factory** worker skill for creating ALL 3 task types in Linear. Invoked by orchestrators (ln-task-coordinator, ln-story-quality-coordinator, ln-test-coordinator) when new tasks needed.
...
- **Implementation tasks** (from ln-task-coordinator): Feature development with Technical Approach
- **Refactoring tasks** (from ln-story-quality-coordinator): Code quality improvements with Regression Testing
- **Test tasks** (from ln-test-coordinator): E2E/Integration/Unit tests with Risk Priority Matrix
...
1. **ln-task-coordinator** (CREATE MODE):
...
2. **ln-story-quality-coordinator** (Path B - Issues Found):
...
3. **ln-test-coordinator** (Pass 1 - Manual Testing Passed):
...
**For implementation (from ln-task-coordinator):**
...
**For refactoring (from ln-story-quality-coordinator):**
...
**For test (from ln-test-coordinator):**
- **Manual Test Results**: Structured comment (Format v1.0) from ln-story-quality-coordinator Pass 1 with AC, Test Results, Edge Cases, Error Handling, Integration
...
**Details:** Each template in `references/` directory (owned by ln-task-creator). See template files for complete section descriptions.
...
**Same validation logic as ln-task-replanner Phase 5.**
...
- Rationale: Full pipeline automation mode (ln-story-processor)
...
NEXT STEPS:
- Run ln-story-validator to verify Story before execution (Backlog → Todo)
- Run ln-story-coordinator to start task execution (Todo → In Progress)
```
...
- Rationale: All new tests created by ln-test-coordinator after manual testing
...
**Rationale**: Code is written during execution (ln-task-executor, ln-task-rework, ln-test-executor), not during planning.
...
- [ ] Next steps provided (ln-story-validator, ln-story-coordinator)
...
- **Used for**: Feature development tasks (from ln-task-coordinator)
...
- **Used for**: Code quality improvement tasks (from ln-story-quality-coordinator)
...
- **Used for**: Story's final test task (from ln-test-coordinator)
...
Phase 4a: Delegate to ln-task-creator
...
**2. ln-story-quality-coordinator (Phase 6 Path B - Issues Found):**
```javascript
...
**3. ln-test-coordinator (Phase 6 - After Manual Testing):**
```javascript
Phase 5: Test Plan Analysis → Priority ≥15 scenarios selected
Phase 6: Invoke ln-task-creator
  Skill(skill: "ln-task-creator", {
    taskType: "test",
    teamId: teamId,
    storyData: {...},
...
- Analyze Story complexity (ln-task-coordinator does this)
- Analyze code quality (ln-story-quality-coordinator does this)
- Perform manual testing (ln-story-quality-coordinator does this)
...
**State**: Always "Backlog" (tasks move to Todo after ln-story-validator approval)
