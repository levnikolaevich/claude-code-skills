...
Fallbacks are valid for Git history, builds, tests, package managers, containers, images, PDFs, notebooks, external websites, binary/media files, paths outside the project root, unsupported languages, unavailable MCP servers, or small markdown/metadata reads where MCP setup adds no value.
