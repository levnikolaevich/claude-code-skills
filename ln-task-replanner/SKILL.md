...
- **Implementation tasks** (from ln-task-coordinator): Feature development tasks when Story AC/Technical Approach changed
- **Refactoring tasks** (from ln-story-quality-coordinator): Code quality improvement tasks when new issues found
- **Test tasks** (from ln-test-coordinator): E2E/Integration/Unit test tasks when manual test results changed
...
**1. ln-task-coordinator (REPLAN MODE):**
...
**3. ln-test-coordinator (Phase 6 - Manual Testing Changed):**
...
Read template from ln-task-creator/references/ based on `taskType` parameter:
...
Generate new doc (taskType determines template) → Validate (same rules as ln-task-creator Phase 2) → `update_issue(description)` → Add comment
...
**Validation rules (type-specific, same as ln-task-creator Phase 2):**
...
**Same validation logic as ln-task-creator Phase 2.**
...
**Location**: [ln-task-replanner/references/replan_algorithm.md](references/replan_algorithm.md)

**Ownership**: ln-task-replanner (worker-specific logic)

**Usage**: ln-task-replanner applies this algorithm in Phase 2 (Compare IDEAL vs Existing)

### Task Templates (ln-task-creator Ownership)

**This skill reads ALL 3 templates** from ln-task-creator/references/ directory:

**task_template_implementation.md:**
- **Purpose**: Implementation task template (7 sections, 146 lines)
- **Location**: [ln-task-creator/references/task_template_implementation.md](../ln-task-creator/references/task_template_implementation.md)
...
- **Location**: [ln-task-creator/references/refactoring_task_template.md](../ln-task-creator/references/refactoring_task_template.md)
...
- **Location**: [ln-task-creator/references/test_task_template.md](../ln-task-creator/references/test_task_template.md)
...
|--------------|---------|----------|------------------|
| ln-task-coordinator | Story AC/approach changed | implementation | idealPlan, guideLinks |
| ln-story-quality-coordinator | New code quality issues | refactoring | codeQualityIssues, refactoringPlan, affectedComponents |
| ln-test-coordinator | Manual test results changed | test | manualTestResults, testPlan, infra, docs, cleanup |
