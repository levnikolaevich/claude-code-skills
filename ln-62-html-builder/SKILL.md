---
name: ln-62-html-builder
description: Builds interactive HTML from project docs with 6 tabs (Overview, Requirements, Architecture/C4, Tech Spec, Roadmap, Guides). Uses Mermaid v11, table structure. Works with ln-61-docs-creator output.
---
...
Use this skill when:
- Have existing project documentation (created by ln-61-docs-creator or manually)
...
1. **Check if presentation exists**:
   ...
   - If NOT exists:
     - Copy all templates from `ln-62-html-builder/references/` → `docs/{folder}/assets/`

2. **Copy template files**:
   ```bash
   cp ln-62-html-builder/references/presentation_template.html → docs/{folder}/assets/
   cp ln-62-html-builder/references/styles.css → docs/{folder}/assets/
   cp ln-62-html-builder/references/scripts.js → docs/{folder}/assets/
   cp ln-62-html-builder/references/build-presentation.js → docs/{folder}/assets/
   cp -r ln-62-html-builder/references/tabs/ → docs/{folder}/assets/tabs/
   ```
...
### Works With:
- **ln-61-docs-creator**: Primary consumer (generates MD docs → this skill → HTML)
- **ln-60-docs-system**: Called by orchestrator after MD generation
- **ln-63-docs-updater**: Re-run after docs updated

### Typical Workflow:
```
1. ln-61-docs-creator
   ↓ (creates MD docs)
2. ln-62-html-builder ← YOU ARE HERE
...
```
1. ln-63-docs-updater
   ↓ (updates MD docs)
2. ln-62-html-builder
...
- [ ] All required template files copied from `ln-62-html-builder/references/` to `docs/{folder}/assets/`:
...
**Dependencies:**
- **Templates**: ln-62-html-builder/references/ (11 files: HTML/CSS/JS templates + mermaid_guidelines.md)
...
- **Works with**: Output from ln-61-docs-creator skill or any standard doc structure
