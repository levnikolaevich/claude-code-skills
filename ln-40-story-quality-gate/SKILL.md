...
- **Step 3:** Manual functional testing against Story AC (delegated to `ln-43-manual-tester`) - FAIL FAST
- **Verdict:** Create test task (via ln-50-story-test-planner) OR refactoring/fix task
...
- Invoked by ln-30-story-executor automatically via Skill tool when test task Done
...
Implementation Tasks (Done)
         ↓
ln-40-story-quality-gate Pass 1 (manual functional testing + code analysis)
         ↓
    [Path A: No issues]
         ↓
ln-50-story-test-planner (creates test task with 11 sections)
         ↓
ln-34-test-executor (Fix Tests → New Tests → Infrastructure → Docs → Cleanup)
         ↓
ln-32-task-reviewer (reviews test task)
         ↓
Test task Done
         ↓
ln-40-story-quality-gate Pass 2 (verify tests → Story Done)

    [Path B: Issues found]
         ↓
Create refactoring task → Execute → ln-40-story-quality-gate Pass 1 again
```

**Critical:** Pass 2 is automatically invoked by ln-30-story-executor when test task status = Done. Can also be invoked manually by user for re-verification.

### Usage Examples

**Automatic invocation (recommended):**
```
# Pass 1 - invoked by ln-30-story-executor when all impl tasks Done
Skill(skill: "ln-40-story-quality-gate", storyId: "US004")

# Pass 2 - invoked by ln-30-story-executor when test task Done
Skill(skill: "ln-40-story-quality-gate", storyId: "US004")
```
...
Skill(skill: "ln-40-story-quality-gate", storyId: "US004", pass: 1)

# Force Pass 2 (verify tests + mark Story Done)
Skill(skill: "ln-40-story-quality-gate", storyId: "US004", pass: 2)
```
...
- Re-run Pass 1 after fixing issues (without waiting for ln-30-story-executor)
...
**Delegate to ln-41-code-quality-checker (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-41-code-quality-checker", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "ISSUES_FOUND") + Linear comment ID
- Details: See [ln-41-code-quality-checker/SKILL.md](../ln-41-code-quality-checker/SKILL.md)

### Phase 3 Verdict: Early Exit Decision

**Parse ln-41-code-quality-checker result:**
...
1. **Create refactoring task:**
   ...
   - Description: Copy issues from ln-41-code-quality-checker result
   - Label: "refactoring"
   - Parent: Story.id
   - Status: Backlog
...
**Delegate to ln-42-regression-checker (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-42-regression-checker", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "FAIL") + Linear comment ID
- Details: See [ln-42-regression-checker/SKILL.md](../ln-42-regression-checker/SKILL.md)

### Phase 4 Verdict: Early Exit Decision

**Parse ln-42-regression-checker result:**
...
1. **Create regression fix task:**
   ...
   - Description: Copy failed test names from ln-42-regression-checker result
   - Label: "bug"
   - Parent: Story.id
   - Status: Backlog
...
**Delegate to ln-43-manual-tester (L3 Worker):**

Invoke via Skill tool:
```
Skill(skill: "ln-43-manual-tester", storyId: Story.id)
```

**Worker responsibilities:**
- Input: Story ID
- Output: JSON verdict ("PASS" | "FAIL") + Linear comment ID (Format v1.0) + temporary script path
- Details: See [ln-43-manual-tester/SKILL.md](../ln-43-manual-tester/SKILL.md)

### Phase 5 Verdict: Early Exit Decision

**Parse ln-43-manual-tester result:**
...
1. **Create bug fix task:**
   ...
   - Description: Copy failed AC and bug details from ln-43-manual-tester result
   - Label: "bug"
   - Parent: Story.id
   - Status: Backlog
...
1. **Check for existing test task:**
   ...
   - **If test task exists:**
     * Check test task status:
       - **Done** → Report: "Test task [TASK-ID] Done. Pass 1 complete. ln-30-story-executor will automatically invoke Pass 2."
       - **In Progress / To Review / Todo** → Report to user: "Test task [TASK-ID] already exists (status: [STATUS]). Wait for completion before running ln-40-story-quality-gate Pass 1 again."
...
2. **Create test task (ONLY if no test task exists):**
   ...
   - **AUTOMATIC invocation:** Invoke ln-50-story-test-planner with `autoApprove: true` (Skill tool) → WAIT completion → test task created (no prompts)
   - Story remains current state until test task Done

3. **Next steps after ln-50-story-test-planner completes:**
   - Test task created in Linear and kanban_board.md
   - Execute test task via ln-34-test-executor (6 steps):
   ...
   - Review test task via ln-32-task-reviewer
   - When test task Done → ln-30-story-executor automatically invokes ln-40-story-quality-gate Pass 2
...
1. **All tests pass:**
   ...
   - Verify 0 failures
   - Verify total tests 10-28 (within enforced limit per `ln-50-story-test-planner/references/risk_based_testing_guide.md`)
...
2. **Update kanban_board.md (minimal cleanup):**
   - Story header already in "### Done (Last 5 tasks)" section (tasks were moved there by ln-32-task-reviewer)
   ...
   - No additional movement needed (tasks already moved by ln-32-task-reviewer)
...
4. **After fix tasks Done:** ln-30-story-executor automatically re-invokes ln-40-story-quality-gate Pass 2
...
**ln-40-story-quality-gate responsibility**: Manages **1 of 4** Story status transitions (Final approval phase).

| Story Status Transition | Responsible Skill | When |
|-------------------------|-------------------|------|
| Backlog → Todo | ln-20-story-validator | After auto-fix and approval |
| Todo → In Progress | ln-30-story-executor | First task execution starts |
| In Progress → To Review | ln-30-story-executor | All tasks Done |
| **To Review → Done** | **ln-40-story-quality-gate Pass 2** | **All tests verified, Priority ≥15 covered** |

**Why this matters**:
- ln-40-story-quality-gate ONLY updates status in Pass 2 (Phase 3: Mark Story Done)
- ln-40-story-quality-gate does NOT handle other transitions (Backlog → Todo, Todo → In Progress, In Progress → To Review)
- Clear ownership prevents duplicate updates from multiple skills
- ln-40-story-quality-gate Pass 1 does NOT update Story status - it creates test/refactor tasks only

### L2→L2 Delegation: Invoked by ln-30-story-executor
...
|------------|------|-------------------|-----------|
| **ln-30-story-executor → ln-40-story-quality-gate Pass 1** | After all implementation tasks Done | Task execution → Story quality validation | ln-30-story-executor delegates quality verification (Code Quality, Regression, Manual Testing, Early Exit Pattern) |
| **ln-30-story-executor → ln-40-story-quality-gate Pass 2** | After test task Done | Task execution → Final approval | ln-30-story-executor delegates final Story approval (test verification, Priority ≥15 coverage, Story → Done) |
...
- Pass 1 Phase 5: Delegates manual testing to ln-43-manual-tester (curl/puppeteer, Linear comments captured there)
- Pass 1 Phase 6: **Delegates to ln-50-story-test-planner** via Skill tool (test task creation)
- Orchestrates quality validation flow with Early Exit Pattern (fail fast at each phase)

**Compliance with L2→L2 Rules:**
- ✅ Rule 1: Explicit Skill tool invocation (invoked by ln-30-story-executor via `Skill(skill: "ln-40-story-quality-gate")`)
...
- ✅ Rule 4: DAG only (never calls back to ln-30-story-executor, only creates tasks for it to execute)
- ✅ Rule 5: Documented in both SKILL.md files

**Self-Healing Pipeline Integration:**
- Pass 1 creates fix/refactor/test tasks in Backlog → ln-30-story-executor loops back to Phase 3
- Pass 2 creates fix tasks if tests fail → ln-30-story-executor loops back to Phase 3
...
- [ ] ln-41-code-quality-checker invoked via Skill tool
...
- [ ] ln-42-regression-checker invoked via Skill tool
...
- [ ] ln-43-manual-tester invoked via Skill tool
- [ ] Worker result received: JSON verdict ("PASS" | "FAIL")
...
- [ ] **If test task does NOT exist:** Proceeded to ln-50-story-test-planner invocation

**✅ Verdict Determined (Phase 6):**
- [ ] **All Phase 3-4-5 passed** (Code Quality → Regression → Manual Testing)
- [ ] ln-50-story-test-planner invoked AUTOMATICALLY with autoApprove: true (ONLY if no test task exists)
...
- [ ] **Pass:**
  ...
  - kanban_board.md minimal cleanup:
    * Story header already in "### Done (Last 5 tasks)" section (tasks moved there by ln-32-task-reviewer)
...
5. **Phase 6: E2E first** - Use Skill tool to invoke ln-50-story-test-planner (Risk-Based Testing)
...
| Aspect | ln-20-story-validator | ln-32-task-reviewer | ln-40-story-quality-gate Pass 1 | ln-40-story-quality-gate Pass 2 |
|--------|-------------------|-----------------|-------------------------|-------------------------|
