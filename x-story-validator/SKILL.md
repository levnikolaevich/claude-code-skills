...
- Does NOT execute implementation (delegates to x-story-coordinator)
...
3. **Check documentation gaps:**
   ...
   c) **Auto-create missing documentation:**
      - **Guides:** Invoke x-guide-creator (HOW to implement patterns)
      - **Manuals:** Invoke x-manual-creator (HOW to use libraries/APIs)
      - **ADRs:** Invoke x-adr-creator (WHY decisions - AUTO-RESEARCH from Story)
...
10. **Test Task Cleanup (#10)** - Remove premature test tasks (created later by x-test-coordinator)
...
13. **Story Size & Task Granularity (#9)** - Check 3-8 tasks, 3-5h each; invoke x-task-coordinator if < 3 tasks
...
**Reference:** See `x-test-coordinator/references/risk_based_testing_guide.md` for complete methodology.
...
- No test task exists (created later by x-test-coordinator)

**Auto-fix actions:**
- **If < 3 tasks:**
  - Analyze Story Context and Technical Notes
  - Check Consumer-First gaps (Consumer → Service → Repository)
  - Invoke x-task-coordinator via Skill tool: `Skill(skill: "x-task-coordinator")`
  ...
  - Add comment: "Missing implementation tasks created via x-task-coordinator"
...
- For EACH test task found:
  ...
  - Add comment: "Test task removed - will be created by x-test-coordinator after manual testing (Pass 1)"
...
**Rationale:** Test task created AFTER manual testing (by x-test-coordinator), not before. Manual testing reveals real scenarios for comprehensive test coverage.
...
**Purpose:** x-task-executor reads these links when implementing.
...
- Phase 3 (#9): Detect < 3 tasks violation
  ...
  - Invoke x-task-coordinator: `Skill(skill: "x-task-coordinator")`
  - x-task-coordinator creates 2 missing tasks (Service, Repository)
...
- `../x-task-coordinator/references/task_template_universal.md` - Task structure (7 sections)

**Checklists (Verification):**
- `verification_checklist.md` - Complete 16-criteria checklist with detailed checks

**Methodology (Testing):**
- `../x-test-coordinator/references/risk_based_testing_guide.md` - Risk-Based Testing approach (Priority ≥15 scenarios)
...
4. **Delegate task creation** - Use x-task-coordinator Skill tool (Phase 3 #9), never create tasks directly
...
- [ ] **Workflow Optimization (9-12):**
  ...
  - Test tasks removed (created later by x-test-coordinator) (#10)
  - Documentation integrated into implementation tasks (#8)
  - Tasks reordered per Consumer-First principle (#14)
- [ ] **Scope & Quality (13-16):**
  - Story size validated (3-8 tasks, x-task-coordinator invoked if < 3) (#9)
