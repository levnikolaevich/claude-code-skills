...
- Does NOT execute implementation (delegates to ln-30-story-executor)
- Does NOT create/update ADRs (delegates to ln-22-adr-creator)
...
3. **Check documentation gaps:**
   ...
   c) **Auto-create missing documentation:**
      - **Guides:** Invoke ln-21-guide-creator (HOW to implement patterns)
      - **Manuals:** Invoke ln-23-manual-creator (HOW to use libraries/APIs)
      - **ADRs:** Invoke ln-22-adr-creator (WHY decisions - AUTO-RESEARCH from Story)
...
10. **Test Task Cleanup (#10)** - Remove premature test tasks (created later by ln-50-story-test-planner)
...
13. **Story Size & Task Granularity (#9)** - Check 3-8 tasks, 3-5h each; invoke ln-10-story-decomposer if < 3 tasks
...
**Reference:** See `ln-50-story-test-planner/references/risk_based_testing_guide.md` for complete methodology.
...
- No test task exists (created later by ln-50-story-test-planner)

**Auto-fix actions:**
- **If < 3 tasks:**
  - Analyze Story Context and Technical Notes
  - Check Consumer-First gaps (Consumer → Service → Repository)
  - Invoke ln-10-story-decomposer via Skill tool: `Skill(skill: "ln-10-story-decomposer")`
  ...
  - Add comment: "Missing implementation tasks created via ln-10-story-decomposer"
...
- For EACH test task found:
  ...
  - Add comment: "Test task removed - will be created by ln-50-story-test-planner after manual testing (Pass 1)"
...
**Rationale:** Test task created AFTER manual testing (by ln-50-story-test-planner), not before. Manual testing reveals real scenarios for comprehensive test coverage.
...
**Purpose:** ln-31-task-executor reads these links when implementing.
...
- Phase 3 (#9): Detect < 3 tasks violation
  ...
  - Invoke ln-10-story-decomposer: `Skill(skill: "ln-10-story-decomposer")`
  - ln-10-story-decomposer creates 2 missing tasks (Service, Repository)
...
- `../ln-10-story-decomposer/references/task_template_universal.md` - Task structure (7 sections)

**Checklists (Verification):**
- `verification_checklist.md` - Complete 16-criteria checklist with detailed checks

**Methodology (Testing):**
- `../ln-50-story-test-planner/references/risk_based_testing_guide.md` - Risk-Based Testing approach (Priority ≥15 scenarios)

**Integration (Linear API):**
- `../ln-70-epic-creator/references/linear_integration.md` - Linear MCP usage patterns
...
4. **Delegate task creation** - Use ln-10-story-decomposer Skill tool (Phase 3 #9), never create tasks directly
...
- [ ] Missing guides auto-created via ln-21-guide-creator
...
- [ ] **Workflow Optimization (9-12):**
  ...
  - Test tasks removed (created later by ln-50-story-test-planner) (#10)
  - Documentation integrated into implementation tasks (#8)
  - Tasks reordered per Consumer-First principle (#14)
- [ ] **Scope & Quality (13-16):**
  - Story size validated (3-8 tasks, ln-10-story-decomposer invoked if < 3) (#9)
...
**Version:** 11.0.0 (3-Type Documentation Integration)
