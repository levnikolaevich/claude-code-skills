---
name: x-test-coordinator
description: Plans Story test task by Risk-Based Testing after manual testing. Calculates priorities, selects E2E/Integration/Unit, delegates to x-task-creator. Invoked by x-story-quality-coordinator.
---
...
- **Invoked by x-story-quality-coordinator Pass 1** after manual functional testing PASSED
- **Invocation method:** Use Skill tool with command: `Skill(command: "x-test-coordinator")`
...
Do NOT use if:
- Manual testing NOT completed → Wait for x-story-quality-coordinator Pass 1
- Manual test results NOT in Linear comment → x-story-quality-coordinator must document first
...
Invoke x-task-creator worker with taskType: "test"
...
Invoke x-task-replanner worker with taskType: "test"
...
- [ ] CREATE MODE (if count = 0): Delegated to x-task-creator with taskType: "test"
- [ ] REPLAN MODE (if count ≥ 1): Delegated to x-task-replanner with taskType: "test"
...
- [ ] x-task-creator: Test task created in Linear + kanban_board.md updated
...
**Location**: Moved to [x-task-creator/references/test_task_template.md](../x-task-creator/references/test_task_template.md)
...
**Usage**: Workers (x-task-creator, x-task-replanner) read this template when generating test task documents (via `taskType: "test"`)

### linear_integration.md (Shared Reference)

**Location**: [x-epic-creator/references/linear_integration.md](../x-epic-creator/references/linear_integration.md)
