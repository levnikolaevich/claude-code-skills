---
name: ln-docs-system
description: Creates complete doc system via ln-docs-creator + ln-html-builder + ln-adr-creator. Used when starting new project and need full documentation suite at once.
---
...
1. [ln-docs-creator](../ln-docs-creator/SKILL.md) - Create MD docs
2. [ln-html-builder](../ln-html-builder/SKILL.md) - Create HTML presentation (optional)
3. [ln-adr-creator](../ln-adr-creator/SKILL.md) - Add individual ADRs later (optional)
...
1. Show user what will be created:
   - Project documentation (MD files via ln-docs-creator)
   - HTML presentation (via ln-html-builder)
...
**Invocation:** `Skill(skill: "ln-docs-creator")` → AUTOMATIC
...
**Invocation:** `Skill(skill: "ln-html-builder")` → AUTOMATIC (no user input)

**Expected output:** `presentation_final.html` + `assets/` directory

**Skip:** If "no" → run ln-html-builder later manually
...
3. Recommend next steps:
   ...
   - "Run ln-epic-creator to decompose scope into Epics"
   - "Share documentation with technical stakeholders"
   - "Run ln-adr-creator skill to add more ADRs later (if needed)"
...
│   └── ...
├── presentation_final.html           # ← Interactive HTML (optional, generated by ln-html-builder)
└── assets/                           # ← HTML modules (optional, generated by ln-html-builder)
```
...
1. **ln-docs-system** (this skill) - Create complete documentation system
2. **ln-epic-creator** - Decompose scope into Epics (Linear Projects)
3. **ln-story-manager** - Create User Stories for each Epic (automatic decomposition + replan)
4. **ln-task-coordinator** - Break down Stories into implementation tasks (automatic decomposition + replan)
5. **ln-story-validator** - Verify Stories before development
6. **ln-story-coordinator** - Orchestrate Story implementation
7. **ln-story-quality-coordinator** - Review completed Stories
...
**Trade-offs:**
- ⚠️ Less granular control (can't skip ln-docs-creator phases)
- ⚠️ Longer execution time (15-20 minutes)
- ⚠️ Can't reuse existing docs (starts fresh)

**When to use manual approach instead:**
- Need to update existing documentation → use [ln-docs-updater](../ln-docs-updater/SKILL.md)
- Need only HTML rebuild → use [ln-html-builder](../ln-html-builder/SKILL.md)
- Need one specific ADR → use [ln-adr-creator](../ln-adr-creator/SKILL.md)
...
- [ ] Workflow explained to user:
  - Project documentation (MD files via ln-docs-creator)
  - HTML presentation (via ln-html-builder, optional)
  - Estimated time: 15-20 minutes with interactive dialog
...
- [ ] User notified: "Invoking ln-docs-creator skill..."
- [ ] ln-docs-creator invoked via Skill tool (command: "ln-docs-creator")
- [ ] ln-docs-creator completed all 5 phases:
...
- [ ] **If user chose "yes" for HTML:**
  - User notified: "Invoking ln-html-builder skill..."
  - ln-html-builder invoked via Skill tool (command: "ln-html-builder")
  - ln-html-builder completed all 3 phases:
...
- [ ] **If user chose "no":**
  ...
  - User notified: "Skipped HTML presentation (can run ln-html-builder later)"
...
- [ ] Next steps recommended:
  ...
  - "Share documentation with technical stakeholders"
  - "Run ln-adr-creator skill to add more ADRs later (if needed)"
- [ ] Integration workflow mentioned (ln-docs-system → ln-epic-creator → ln-story-manager → ...)
...
- Next steps recommendation (ln-epic-creator to decompose scope)
