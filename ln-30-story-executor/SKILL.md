---
name: ln-30-story-executor
description: Executes Story by orchestrating child tasks. Prioritizes To Review (→ ln-32-task-reviewer), then Todo (→ ln-31-task-executor/test). Manages transitions (Todo → In Progress → Done). Auto-discovers team/config.
---
...
**Note:** This skill orchestrates full Story execution including explicit delegation to ln-40-story-quality-gate Pass 1 (when all implementation tasks Done) and Pass 2 (when test task Done).

## Workflow Overview

```
┌─────────────────────────────────────────────────────┐
│ ln-30-story-executor (TASK ORCHESTRATOR)                │
├─────────────────────────────────────────────────────┤
│ Phase 1: Discovery                                  │
│ Phase 2: Load Story & Task Metadata                 │
│                                                     │
│ Phase 3: Task Orchestration Loop ◄──────────┐       │
│   Priority 0: To Review → ln-32-task-reviewer   │       │
│   Priority 1: To Rework → ln-33-task-rework     │       │
...
│   → Invoke ln-40-story-quality-gate Pass 1          │       │
│     (Skill tool - Story quality worker)     │       │
│                                                     │
│   Pass 1 executes:                                  │
│     - ln-41-code-quality-checker (Fail Fast)            │
│     - ln-42-regression-checker (Fail Fast)              │
│     - ln-43-manual-tester (Fail Fast, Format v1.0)      │
│     - Verdict: Path A/B/C                           │
│                                                     │
│   Path A: Test task created (via ln-50-story-test-planner │
│           → ln-11-task-creator/replanner)               │
...
**Key Points:**
- **Orchestrator-Worker Pattern:** ln-30-story-executor orchestrates tasks, delegates Story quality to ln-40-story-quality-gate
- **Task-Level Orchestration:** Phase 3 manages task workflow (To Review → To Rework → Todo)
- **Story-Level Delegation:** Phase 4 delegates to ln-40-story-quality-gate Pass 1 for quality verification
- **Self-Healing Loop:** ln-40-story-quality-gate creates fix/refactor/test tasks → loops back to Phase 3
...
- ln-30-story-executor ONLY invokes executor skills with task ID parameter
- ln-30-story-executor does NOT update task status directly
- Executor skills (ln-31-task-executor, ln-34-test-executor, ln-33-task-rework, ln-32-task-reviewer) are SOLELY responsible for updating their selected task status
...
- **Phase 4:** Story-level delegation (invokes ln-40-story-quality-gate Pass 1 for quality verification)
- ln-30-story-executor does NOT execute Story quality checks directly - delegates to ln-40-story-quality-gate
...
- If tasks in To Review exist (from Linear metadata loaded in Phase 2):
  ...
  3. **Use Skill tool:** `Skill(skill: "ln-32-task-reviewer")` with task ID
  4. ln-32-task-reviewer loads full description and handles: To Review → Done OR To Review → To Rework
...
- If no To Review tasks, but To Rework tasks exist (from Linear metadata loaded in Phase 2):
  ...
  3. **Use Skill tool:** `Skill(skill: "ln-33-task-rework")` with task ID
  4. ln-33-task-rework loads full description and handles: To Rework → In Progress → To Review
  5. After completion: Loop back to Phase 2 (reload from Linear - AUTOMATIC, no user input)
...
- If no To Review AND no To Rework tasks, but Todo tasks exist (from Linear metadata loaded in Phase 2):
  ...
  4. **Use Skill tool based on task type:**
     - **IF label "tests":** `Skill(skill: "ln-34-test-executor")` with task ID
       - ln-34-test-executor loads full test task description (11 sections from ln-50-story-test-planner task plan)
       - Handles Story Finalizer test task: Todo → In Progress → To Review
     - **ELSE (implementation):** `Skill(skill: "ln-31-task-executor")` with task ID
       - ln-31-task-executor loads full implementation task description (7 sections)
...
Exit when no tasks in these statuses → Phase 4 → Invoke ln-40-story-quality-gate Pass 1 for Story-level quality verification.
...
**Goal:** Delegate Story-level quality verification to ln-40-story-quality-gate Pass 1

**Architecture:** Orchestrator-Worker Pattern - ln-30-story-executor (task orchestrator) delegates story quality to ln-40-story-quality-gate (story worker)
...
**2. Invoke ln-40-story-quality-gate Pass 1:**
```
Skill(skill: "ln-40-story-quality-gate")
```
- Pass Story ID as parameter
- ln-40-story-quality-gate executes 6 phases:
...
- ln-40-story-quality-gate returns verdict + created task IDs (if any)

**3. Process ln-40-story-quality-gate response:**

**Path A: Test task created**
- ln-40-story-quality-gate created test task (taskType: "test") via ln-50-story-test-planner
- Test task status = Backlog
- **Auto-verify:** Invoke ln-20-story-validator with Story ID and test task ID (Backlog → Todo)
- Add comment to Story: "ln-40-story-quality-gate Pass 1 completed. Test task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute test task through orchestration loop
  - Priority 2: ln-34-test-executor invoked (test task has label "tests")
...
- ln-40-story-quality-gate found code quality issues
- Refactoring task created (taskType: "refactoring") via ln-11-task-creator
- Refactor task status = Backlog
- **Auto-verify:** Invoke ln-20-story-validator with Story ID and refactor task ID (Backlog → Todo)
- Add comment to Story: "ln-40-story-quality-gate Pass 1 completed. Refactoring task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute refactor task through orchestration loop
  - Priority 2: ln-31-task-executor invoked
...
- ln-40-story-quality-gate Pass 1 passed without creating tasks (rare case)
...
**4. Invoke ln-40-story-quality-gate Pass 2 (explicit delegation):**
...
3. Invoke ln-40-story-quality-gate Pass 2:
   ```
   Skill(skill: "ln-40-story-quality-gate")
   ```
4. ln-40-story-quality-gate Pass 2 executes:
...
**When ln-40-story-quality-gate Pass 2 passes:**
- Story status: Done
...
**Output:**
- "ln-30-story-executor completed successfully"
- "Story [ID] status: Done"
- "All tasks completed and verified"

**Note:** ln-30-story-executor now runs FULL pipeline automatically (Todo → In Progress → To Review → Done)

---

### Story Status Responsibility Matrix

**ln-30-story-executor responsibility**: Manages **2 of 4** Story status transitions (Task execution phase).

| Story Status Transition | Responsible Skill | When |
|-------------------------|-------------------|------|
| Backlog → Todo | ln-20-story-validator | After auto-fix and approval |
| **Todo → In Progress** | **ln-30-story-executor** | **First task execution starts (Phase 3 Priority 2)** |
| **In Progress → To Review** | **ln-30-story-executor** | **All tasks Done (Phase 4 transition)** |
| To Review → Done | ln-40-story-quality-gate Pass 2 | All tests verified, Priority ≥15 covered |

**ln-30-story-executor transitions (details):**
...
2. **In Progress → To Review** (Phase 4 Action 4, all tasks Done):
   ...
   - Invoke ln-40-story-quality-gate Pass 2

**Note:** ln-30-story-executor does NOT update To Review → Done (ln-40-story-quality-gate Pass 2 responsibility)

### L2→L2 Delegation: ln-40-story-quality-gate
...
|------------|------|-------------------|-----------|
| **ln-30-story-executor → ln-40-story-quality-gate Pass 1** | Phase 4: All implementation tasks Done | Task execution → Story quality validation | Delegate quality verification (Code Quality, Regression, Manual Testing) to specialized orchestrator |
| **ln-30-story-executor → ln-40-story-quality-gate Pass 2** | Phase 4 Action 4: Test task Done | Task execution → Final approval | Delegate final Story approval (test verification, Priority ≥15 coverage) to specialized orchestrator |

**Why L2→L2 (not L2→L3):**
- ln-40-story-quality-gate is orchestrator (invokes ln-50-story-test-planner, ln-11-task-creator, ln-43-manual-tester)
- Separates task execution (ln-30-story-executor) from quality validation (ln-40-story-quality-gate)
- Enables Self-Healing Pipeline: quality-coordinator creates fix/refactor/test tasks → loops back to ln-30-story-executor Phase 3

**Compliance with L2→L2 Rules:**
- ✅ Rule 1: Explicit Skill tool invocation (`Skill(skill: "ln-40-story-quality-gate")`)
...
**After Phase 4 (ln-40-story-quality-gate Pass 1) completes:**
- "ln-40-story-quality-gate Pass 1 completed: [VERDICT]"
...
- ln-40-story-quality-gate Pass 1 verdict
...
- User must resolve issue manually before re-running ln-30-story-executor
...
- Recommend: "Create tasks using ln-71-story-manager or ln-10-story-decomposer"

### Comparison with Other Skills

| Aspect | ln-30-story-executor | ln-31-task-executor | ln-32-task-reviewer | ln-40-story-quality-gate |
|--------|----------------|----------------|-------------|--------------|
...
| **Invokes** | ln-32-task-reviewer, ln-33-task-rework, ln-31-task-executor, ln-34-test-executor, ln-40-story-quality-gate (via Skill tool) | None | None | ln-50-story-test-planner, ln-11-task-creator, ln-12-task-replanner (via Skill tool) |
| **Status** | Todo → In Progress → To Review | Todo → In Progress → To Review | To Review → Done/Rework | Manual testing + Story Done |
| **When** | User wants to execute entire Story | ln-30-story-executor calls via Skill tool OR user directly | ln-30-story-executor calls via Skill tool OR user directly | ln-30-story-executor calls Pass 1 automatically |
...
- [ ] **Priority 0:** All To Review tasks processed via ln-32-task-reviewer
- [ ] **Priority 1:** All To Rework tasks processed via ln-33-task-rework
- [ ] **Priority 2:** All Todo tasks processed via executors:
  - [ ] Test tasks (label "tests") → ln-34-test-executor
  - [ ] Implementation tasks → ln-31-task-executor
...
- [ ] ln-40-story-quality-gate Pass 1 invoked automatically via Skill tool
- [ ] If ln-40-story-quality-gate creates tasks (test/refactor/fix):
  - [ ] Auto-verify via ln-20-story-validator (Backlog → Todo)
  - [ ] Loop back to Phase 3
- [ ] If test task Done: Story status updated In Progress → To Review
- [ ] ln-40-story-quality-gate Pass 2 invoked automatically via Skill tool
...
- ln-40-story-quality-gate Pass 1: Invoked automatically
- ln-40-story-quality-gate Pass 2: Invoked automatically
- Story lifecycle complete
- Progress report with task summary

---

**Version:** 6.0.0 (BREAKING: Removed Priority 0 Backlog verification - ln-20-story-validator now handles all Backlog tasks. Simplified orchestration to 3 priorities: To Review (0) > To Rework (1) > Todo (2). Maintains auto-invocation of ln-40-story-quality-gate Pass 2 after test task Done.)
