...
**NOT for test tasks:** Use ln-test-executor for Story Finalizer test tasks (11 sections with Fix Existing Tests, Infrastructure, Documentation, Legacy Cleanup)
...
**Note:** Story status management is handled by ln-story-coordinator (orchestrator), not by ln-task-executor (worker). See Separation of Concerns principle.
...
**Output:** Implementation task ready for ln-task-reviewer (status "To Review")
...
**Note:** For Story Finalizer test tasks (11 sections with Fix Existing Tests, Infrastructure, Documentation, Legacy Cleanup), use **ln-test-executor** instead.

---

**Version:** 10.1.0 (Removed Story status update logic - this is now ln-story-coordinator's responsibility per Separation of Concerns principle)
