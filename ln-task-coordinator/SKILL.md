---
name: ln-task-coordinator
description: Orchestrates task operations. Analyzes Story, builds optimal plan (1-6 tasks), delegates to ln-task-creator (CREATE) or ln-task-replanner (REPLAN). Auto-discovers team ID.
---
...
- **Delegates to workers**:
  - No tasks (count = 0) → Invokes **ln-task-creator** (CREATE MODE)
  - Has tasks (count ≥ 1) → Invokes **ln-task-replanner** (REPLAN MODE)
...
**NOT for test tasks** - use ln-test-coordinator for Story's final test task after manual testing.

### When NOT to Use

Do NOT use if:
- Story doesn't exist → Use ln-story-manager first to create Story
- Need to create test tasks → Use ln-test-coordinator after manual testing
...
**ln-task-coordinator is a coordinator** - it does NOT execute work directly:
...
**Workers**:
- **ln-task-creator**: Creates new tasks from IDEAL plan (CREATE MODE)
- **ln-task-replanner**: Updates existing tasks when requirements change (REPLAN MODE)
...
4. Delegate: count = 0 → ln-task-creator, count ≥ 1 → ln-task-replanner
...
See [task_template_implementation.md](../ln-task-creator/references/task_template_implementation.md) for detailed template (owned by ln-task-creator universal factory).
...
**Decision**:
- **Count = 0** → **CREATE MODE** (Phase 4a: Delegate to ln-task-creator)
- **Count ≥ 1** → **REPLAN MODE** (Phase 4b: Delegate to ln-task-replanner)

### Phase 4a: Delegate to ln-task-creator (if Count = 0)

No existing tasks found. Delegate to ln-task-creator worker.

**Invocation**:
```
Skill(skill: "ln-task-creator", {
...
- Details: See [ln-task-creator/SKILL.md](../ln-task-creator/SKILL.md)

### Phase 4b: Delegate to ln-task-replanner (if Count ≥ 1)

Existing tasks found. Delegate to ln-task-replanner worker.

**Invocation**:
```
Skill(skill: "ln-task-replanner", {
  taskType: "implementation",
  teamId: teamId,
  storyData: {
    id: Story.id,
...
- Details: See [ln-task-replanner/SKILL.md](../ln-task-replanner/SKILL.md)

### Phase 5: Post-Execution

**Verify worker completed successfully**:
- ln-task-creator: Tasks created in Linear + kanban_board.md updated
- ln-task-replanner: Operations executed + kanban_board.md updated
...
**Next Steps**:
- ln-story-validator: Verify Story + Tasks before approval (Backlog → Todo, adds ✅ APPROVED)
...
**Enforced by workers** (ln-task-creator and ln-task-replanner validate):
- Scan for: "write tests", "create tests", "add tests", "unit tests", "E2E tests", "integration tests"
- ERROR if detected → Stop execution

**Why**: All new tests (E2E/Integration/Unit) are created in Story's final test task by ln-test-coordinator AFTER manual testing passes.
...
- ln-task-creator: Generates new descriptions (doesn't need to load)
- ln-task-replanner: Loads existing task descriptions for comparison
...
- [ ] ln-task-creator (if count = 0) OR ln-task-replanner (if count ≥ 1)
...
**Output**: Worker summary + next steps (ln-story-validator)

---

## IDEAL Plan Data Sources

**Where ln-task-coordinator gets information to build IDEAL task plan (Phase 2):**
...
| **Guide Links** | Extract implementation patterns to pass to workers | "Related Guides: Database Access Pattern" → pass link to ln-task-creator |
...
**Location**: Moved to [ln-task-creator/references/task_template_implementation.md](../ln-task-creator/references/task_template_implementation.md)

**Ownership**: ln-task-creator (universal factory owns all product templates)

**Rationale**: Templates moved to universal factory (ln-task-creator) which creates ALL 3 task types (implementation, refactoring, test). ln-task-creator owns all product templates.

**Usage**: Workers (ln-task-creator, ln-task-replanner) read this template when generating implementation task documents (via `taskType: "implementation"`)

### replan_algorithm.md (MOVED)

**Location**: Moved to [ln-task-replanner/references/replan_algorithm.md](../ln-task-replanner/references/replan_algorithm.md)

**Ownership**: ln-task-replanner (worker-specific logic)
...
ln-story-manager: May invoke ln-task-coordinator when creating Stories (optional)

### Calls (via Skill tool)

- **ln-task-creator**: When no existing tasks (CREATE MODE)
- **ln-task-replanner**: When tasks exist (REPLAN MODE)

### Next Steps (User-initiated)

After ln-task-coordinator completes:
- **ln-story-validator**: Verify Story + Tasks before approval (Backlog → Todo)
- **ln-story-coordinator**: Execute Story (orchestrates task execution)
...
- Phase 4a: Invoke ln-task-creator (IDEAL plan + Story data)
  - ln-task-creator: Generate 3 tasks, validate NO tests, show preview, user confirms, create in Linear, update kanban
...
- Phase 4b: Invoke ln-task-replanner (IDEAL plan + Story data + existing task IDs)
  - ln-task-replanner: Load existing, Compare (EP7_01 KEEP, EP7_02 UPDATE +AC4, EP7_03 OBSOLETE), show summary, user confirms, execute operations
