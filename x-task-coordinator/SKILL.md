---
name: x-task-coordinator
description: Orchestrates task operations. Analyzes Story, builds optimal plan (1-6 tasks), delegates to x-task-creator (CREATE) or x-task-replanner (REPLAN). Auto-discovers team ID.
---
...
- **Delegates to workers**:
  - No tasks (count = 0) → Invokes **x-task-creator** (CREATE MODE)
  - Has tasks (count ≥ 1) → Invokes **x-task-replanner** (REPLAN MODE)
...
**NOT for test tasks** - use x-test-coordinator for Story's final test task after manual testing.

### When NOT to Use

Do NOT use if:
- Story doesn't exist → Use x-story-manager first to create Story
- Need to create test tasks → Use x-test-coordinator after manual testing
...
**x-task-coordinator is a coordinator** - it does NOT execute work directly:
...
**Workers**:
- **x-task-creator**: Creates new tasks from IDEAL plan (CREATE MODE)
- **x-task-replanner**: Updates existing tasks when requirements change (REPLAN MODE)
...
4. Delegate: count = 0 → x-task-creator, count ≥ 1 → x-task-replanner
...
See [task_template_implementation.md](../x-task-creator/references/task_template_implementation.md) for detailed template (owned by x-task-creator universal factory).
...
**Decision**:
- **Count = 0** → **CREATE MODE** (Phase 4a: Delegate to x-task-creator)
- **Count ≥ 1** → **REPLAN MODE** (Phase 4b: Delegate to x-task-replanner)

### Phase 4a: Delegate to x-task-creator (if Count = 0)

No existing tasks found. Delegate to x-task-creator worker.

**Invocation**:
```
Skill(skill: "x-task-creator", {
...
- Details: See [x-task-creator/SKILL.md](../x-task-creator/SKILL.md)

### Phase 4b: Delegate to x-task-replanner (if Count ≥ 1)

Existing tasks found. Delegate to x-task-replanner worker.

**Invocation**:
```
Skill(skill: "x-task-replanner", {
  taskType: "implementation",
  teamId: teamId,
  storyData: {
    id: Story.id,
...
- Details: See [x-task-replanner/SKILL.md](../x-task-replanner/SKILL.md)

### Phase 5: Post-Execution

**Verify worker completed successfully**:
- x-task-creator: Tasks created in Linear + kanban_board.md updated
...
**Next Steps**:
- x-story-validator: Verify Story + Tasks before approval (Backlog → Todo, adds ✅ APPROVED)
...
**Enforced by workers** (x-task-creator and x-task-replanner validate):
- Scan for: "write tests", "create tests", "add tests", "unit tests", "E2E tests", "integration tests"
- ERROR if detected → Stop execution

**Why**: All new tests (E2E/Integration/Unit) are created in Story's final test task by x-test-coordinator AFTER manual testing passes.
...
- x-task-creator: Generates new descriptions (doesn't need to load)
- x-task-replanner: Loads existing task descriptions for comparison
...
- [ ] x-task-creator (if count = 0) OR x-task-replanner (if count ≥ 1)
...
**Output**: Worker summary + next steps (x-story-validator)

---

## IDEAL Plan Data Sources

**Where x-task-coordinator gets information to build IDEAL task plan (Phase 2):**
...
| **Guide Links** | Extract implementation patterns to pass to workers | "Related Guides: Database Access Pattern" → pass link to x-task-creator |
...
**Location**: Moved to [x-task-creator/references/task_template_implementation.md](../x-task-creator/references/task_template_implementation.md)

**Ownership**: x-task-creator (universal factory owns all product templates)

**Rationale**: Templates moved to universal factory (x-task-creator) which creates ALL 3 task types (implementation, refactoring, test). x-task-creator owns all product templates.

**Usage**: Workers (x-task-creator, x-task-replanner) read this template when generating implementation task documents (via `taskType: "implementation"`)

### replan_algorithm.md (MOVED)

**Location**: Moved to [x-task-replanner/references/replan_algorithm.md](../x-task-replanner/references/replan_algorithm.md)
...
x-story-manager: May invoke x-task-coordinator when creating Stories (optional)

### Calls (via Skill tool)

- **x-task-creator**: When no existing tasks (CREATE MODE)
- **x-task-replanner**: When tasks exist (REPLAN MODE)

### Next Steps (User-initiated)

After x-task-coordinator completes:
- **x-story-validator**: Verify Story + Tasks before approval (Backlog → Todo)
- **x-story-coordinator**: Execute Story (orchestrates task execution)
...
- Phase 4a: Invoke x-task-creator (IDEAL plan + Story data)
  - x-task-creator: Generate 3 tasks, validate NO tests, show preview, user confirms, create in Linear, update kanban
...
- Phase 4b: Invoke x-task-replanner (IDEAL plan + Story data + existing task IDs)
  - x-task-replanner: Load existing, Compare (EP7_01 KEEP, EP7_02 UPDATE +AC4, EP7_03 OBSOLETE), show summary, user confirms, execute operations
