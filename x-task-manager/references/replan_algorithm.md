...
Detailed comparison logic for x-task-manager REPLAN MODE. This algorithm determines which operations (KEEP/UPDATE/OBSOLETE/CREATE) to perform when existing tasks are found for a Story.

## Overview

When a Story's requirements change and tasks already exist, x-task-manager compares the IDEAL task decomposition (from Phase 2) with existing tasks (from Linear) to determine the minimal set of operations needed.
