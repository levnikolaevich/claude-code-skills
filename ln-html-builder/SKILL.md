---
name: ln-html-builder
description: Builds interactive HTML from project docs with 6 tabs (Overview, Requirements, Architecture/C4, Tech Spec, Roadmap, Guides). Uses Mermaid v11, table structure. Works with ln-docs-creator output.
---
...
Use this skill when:
- Have existing project documentation (created by ln-docs-creator or manually)
...
1. **Check if presentation exists**:
   ...
   - If NOT exists:
     - Copy all templates from `ln-html-builder/references/` → `docs/{folder}/assets/`

2. **Copy template files**:
   ```bash
   cp ln-html-builder/references/presentation_template.html → docs/{folder}/assets/
   cp ln-html-builder/references/styles.css → docs/{folder}/assets/
   cp ln-html-builder/references/scripts.js → docs/{folder}/assets/
   cp ln-html-builder/references/build-presentation.js → docs/{folder}/assets/
   cp -r ln-html-builder/references/tabs/ → docs/{folder}/assets/tabs/
   ```
...
### Works With:
- **ln-docs-creator**: Primary consumer (generates MD docs → this skill → HTML)
- **ln-docs-system**: Called by orchestrator after MD generation
- **ln-docs-updater**: Re-run after docs updated

### Typical Workflow:
```
1. ln-docs-creator
   ↓ (creates MD docs)
2. ln-html-builder ← YOU ARE HERE
...
- [ ] All required template files copied from `ln-html-builder/references/` to `docs/{folder}/assets/`:
...
**Dependencies:**
- **Templates**: ln-html-builder/references/ (11 files: HTML/CSS/JS templates + mermaid_guidelines.md)
...
- **Works with**: Output from ln-docs-creator skill or any standard doc structure
