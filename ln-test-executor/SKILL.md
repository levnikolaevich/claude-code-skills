...
Execute approved Story Finalizer test tasks (Todo → In Progress → To Review) created by ln-test-coordinator with comprehensive 11-section structure.

## When to Use This Skill

This skill should be used when executing Story Finalizer test task (status = Todo, created by ln-test-coordinator) with 11 sections:
...
**Note:** For implementation tasks (without test logic), use **ln-task-executor** skill instead.
...
**Note:** Risk assessment completed by ln-test-coordinator. Task contains exact scenarios with Priority scores.
...
**Reference:** See `ln-test-coordinator/references/risk_based_testing_guide.md` for complete Risk-Based Testing methodology, test limits (2-5/3-8/5-15, total 10-28 max), and test selection criteria.
...
**Output:** Story Finalizer test task ready for ln-task-reviewer (status "To Review")
...
3. **Follow task description:** All 11 sections contain specific instructions from ln-test-coordinator analysis
