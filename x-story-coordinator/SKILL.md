---
name: x-story-coordinator
description: Executes Story by orchestrating child tasks. Prioritizes To Review (→ x-task-reviewer), then Todo (→ x-task-executor/test). Manages transitions (Todo → In Progress → Done). Auto-discovers team/config.
---
...
**Note:** This skill orchestrates full Story execution including explicit delegation to x-story-quality-coordinator Pass 1 (when all implementation tasks Done) and Pass 2 (when test task Done).

## Workflow Overview

```
┌─────────────────────────────────────────────────────┐
│ x-story-coordinator (TASK ORCHESTRATOR)                │
├─────────────────────────────────────────────────────┤
│ Phase 1: Discovery                                  │
│ Phase 2: Load Story & Task Metadata                 │
│                                                     │
│ Phase 3: Task Orchestration Loop ◄──────────┐       │
│   Priority 0: To Review → x-task-reviewer   │       │
│   Priority 1: To Rework → x-task-rework     │       │
...
│   → Invoke x-story-quality-coordinator Pass 1          │       │
...
**Key Points:**
- **Orchestrator-Worker Pattern:** x-story-coordinator orchestrates tasks, delegates Story quality to x-story-quality-coordinator
- **Task-Level Orchestration:** Phase 3 manages task workflow (To Review → To Rework → Todo)
- **Story-Level Delegation:** Phase 4 delegates to x-story-quality-coordinator Pass 1 for quality verification
- **Self-Healing Loop:** x-story-quality-coordinator creates fix/refactor/test tasks → loops back to Phase 3
...
- x-story-coordinator ONLY invokes executor skills with task ID parameter
- x-story-coordinator does NOT update task status directly
...
- **Phase 4:** Story-level delegation (invokes x-story-quality-coordinator Pass 1 for quality verification)
- x-story-coordinator does NOT execute Story quality checks directly - delegates to x-story-quality-coordinator
...
Exit when no tasks in these statuses → Phase 4 → Invoke x-story-quality-coordinator Pass 1 for Story-level quality verification.
...
**Goal:** Delegate Story-level quality verification to x-story-quality-coordinator Pass 1

**Architecture:** Orchestrator-Worker Pattern - x-story-coordinator (task orchestrator) delegates story quality to x-story-quality-coordinator (story worker)
...
**2. Invoke x-story-quality-coordinator Pass 1:**
```
Skill(skill: "x-story-quality-coordinator")
```
- Pass Story ID as parameter
- x-story-quality-coordinator executes 6 phases:
...
- x-story-quality-coordinator returns verdict + created task IDs (if any)

**3. Process x-story-quality-coordinator response:**

**Path A: Test task created**
- x-story-quality-coordinator created test task (taskType: "test") via x-test-coordinator
- Test task status = Backlog
- **Auto-verify:** Invoke x-story-validator with Story ID and test task ID (Backlog → Todo)
- Add comment to Story: "x-story-quality-coordinator Pass 1 completed. Test task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute test task through orchestration loop
  - Priority 2: x-test-executor invoked (test task has label "tests")
...
- x-story-quality-coordinator found code quality issues
- Refactoring task created (taskType: "refactoring") via x-task-creator
- Refactor task status = Backlog
- **Auto-verify:** Invoke x-story-validator with Story ID and refactor task ID (Backlog → Todo)
- Add comment to Story: "x-story-quality-coordinator Pass 1 completed. Refactoring task created: [TASK_ID]. Auto-verified and moved to Todo."
- **GOTO Phase 3:** Execute refactor task through orchestration loop
  - Priority 2: x-task-executor invoked
...
- x-story-quality-coordinator Pass 1 passed without creating tasks (rare case)
...
**4. Invoke x-story-quality-coordinator Pass 2 (explicit delegation):**
...
3. Invoke x-story-quality-coordinator Pass 2:
   ```
   Skill(skill: "x-story-quality-coordinator")
   ```
4. x-story-quality-coordinator Pass 2 executes:
...
**When x-story-quality-coordinator Pass 2 passes:**
- Story status: Done
...
**Output:**
- "x-story-coordinator completed successfully"
- "Story [ID] status: Done"
- "All tasks completed and verified"

**Note:** x-story-coordinator now runs FULL pipeline automatically (Todo → In Progress → To Review → Done)

---

### Story Status Responsibility Matrix

**x-story-coordinator responsibility**: Manages **2 of 4** Story status transitions (Task execution phase).

| Story Status Transition | Responsible Skill | When |
|-------------------------|-------------------|------|
| Backlog → Todo | x-story-validator | After auto-fix and approval |
| **Todo → In Progress** | **x-story-coordinator** | **First task execution starts (Phase 3 Priority 2)** |
| **In Progress → To Review** | **x-story-coordinator** | **All tasks Done (Phase 4 transition)** |
| To Review → Done | x-story-quality-coordinator Pass 2 | All tests verified, Priority ≥15 covered |

**x-story-coordinator transitions (details):**
...
2. **In Progress → To Review** (Phase 4 Action 4, all tasks Done):
   ...
   - Invoke x-story-quality-coordinator Pass 2

**Note:** x-story-coordinator does NOT update To Review → Done (x-story-quality-coordinator Pass 2 responsibility)

### L2→L2 Delegation: x-story-quality-coordinator
...
|------------|------|-------------------|-----------|
| **x-story-coordinator → x-story-quality-coordinator Pass 1** | Phase 4: All implementation tasks Done | Task execution → Story quality validation | Delegate quality verification (Code Quality, Regression, Manual Testing) to specialized orchestrator |
| **x-story-coordinator → x-story-quality-coordinator Pass 2** | Phase 4 Action 4: Test task Done | Task execution → Final approval | Delegate final Story approval (test verification, Priority ≥15 coverage) to specialized orchestrator |

**Why L2→L2 (not L2→L3):**
- x-story-quality-coordinator is orchestrator (invokes x-test-coordinator, x-task-creator, x-manual-tester)
- Separates task execution (x-story-coordinator) from quality validation (x-story-quality-coordinator)
- Enables Self-Healing Pipeline: quality-coordinator creates fix/refactor/test tasks → loops back to x-story-coordinator Phase 3

**Compliance with L2→L2 Rules:**
- ✅ Rule 1: Explicit Skill tool invocation (`Skill(skill: "x-story-quality-coordinator")`)
...
**After Phase 4 (x-story-quality-coordinator Pass 1) completes:**
- "x-story-quality-coordinator Pass 1 completed: [VERDICT]"
...
- x-story-quality-coordinator Pass 1 verdict
...
- User must resolve issue manually before re-running x-story-coordinator
...
- Recommend: "Create tasks using x-story-manager or x-task-coordinator"

### Comparison with Other Skills

| Aspect | x-story-coordinator | x-task-executor | x-task-reviewer | x-story-quality-coordinator |
|--------|----------------|----------------|-------------|--------------|
...
| **Invokes** | x-task-reviewer, x-task-rework, x-task-executor, x-test-executor, x-story-quality-coordinator (via Skill tool) | None | None | x-test-coordinator, x-task-creator, x-task-replanner (via Skill tool) |
| **Status** | Todo → In Progress → To Review | Todo → In Progress → To Review | To Review → Done/Rework | Manual testing + Story Done |
| **When** | User wants to execute entire Story | x-story-coordinator calls via Skill tool OR user directly | x-story-coordinator calls via Skill tool OR user directly | x-story-coordinator calls Pass 1 automatically |
...
- [ ] **Priority 0:** All To Review tasks processed via x-task-reviewer
- [ ] **Priority 1:** All To Rework tasks processed via x-task-rework
- [ ] **Priority 2:** All Todo tasks processed via executors:
  - [ ] Test tasks (label "tests") → x-test-executor
  - [ ] Implementation tasks → x-task-executor
...
- [ ] x-story-quality-coordinator Pass 1 invoked automatically via Skill tool
- [ ] If x-story-quality-coordinator creates tasks (test/refactor/fix):
  - [ ] Auto-verify via x-story-validator (Backlog → Todo)
  - [ ] Loop back to Phase 3
- [ ] If test task Done: Story status updated In Progress → To Review
- [ ] x-story-quality-coordinator Pass 2 invoked automatically via Skill tool
...
- x-story-quality-coordinator Pass 1: Invoked automatically
- x-story-quality-coordinator Pass 2: Invoked automatically
- Story lifecycle complete
- Progress report with task summary

---

**Version:** 6.0.0 (BREAKING: Removed Priority 0 Backlog verification - x-story-validator now handles all Backlog tasks. Simplified orchestration to 3 priorities: To Review (0) > To Rework (1) > Todo (2). Maintains auto-invocation of x-story-quality-coordinator Pass 2 after test task Done.)
